    ...
    <div class="py-6" style="background-color: #FFF8F3;">
        <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="overflow-hidden rounded-2xl bg-white p-6 shadow-lg border border-[#F5A623]">
                <h2 class="mb-4 text-xl font-extrabold tracking-tight text-[#2C1810]">Edit Kurir / Driver</h2>

                <form method="POST" action="{{ route('admin.kurir.update', $kurir->id_kurir) }}" class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    @csrf
                    @method('PUT')

                    <div>
                        <label class="block text-sm font-semibold text-[#2C1810]">Nama Kurir</label>
                        <input type="text" name="nama_kurir" value="{{ old('nama_kurir', $kurir->nama_kurir) }}" class="mt-1 block w-full rounded-lg border-[#F5A623] text-sm text-[#2C1810] shadow-sm focus:border-[#E8612A] focus:ring-[#E8612A]" required>
                    ...
                    <div>
                        ...
                        <input type="text" name="no_telp" value="{{ old('no_telp', $kurir->no_telp) }}" class="mt-1 block w-full rounded-lg border-[#F5A623] text-sm text-[#2C1810] shadow-sm focus:border-[#E8612A] focus:ring-[#E8612A]" required>
                    ...
                    <div>
                        ...
                        <select name="jenis_kendaraan" class="mt-1 block w-full rounded-lg border-[#F5A623] text-sm text-[#2C1810] shadow-sm focus:border-[#E8612A] focus:ring-[#E8612A]" required>
                            <option value="motor" @selected(old('jenis_kendaraan', $kurir->jenis_kendaraan) === 'motor')>Motor</option>
                            <option value="mobil" @selected(old('jenis_kendaraan', $kurir->jenis_kendaraan) === 'mobil')>Mobil</option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="plat_kendaraan" value="{{ old('plat_kendaraan', $kurir->plat_kendaraan) }}" class="mt-1 block w-full rounded-lg border-[#F5A623] text-sm text-[#2C1810] shadow-sm focus:border-[#E8612A] focus:ring-[#E8612A]">
                    ...
                    <div>
                        ...
                        <select name="status" class="mt-1 block w-full rounded-lg border-[#F5A623] text-sm text-[#2C1810] shadow-sm focus:border-[#E8612A] focus:ring-[#E8612A]" required>
                            <option value="aktif" @selected(old('status', $kurir->status) === 'aktif')>Aktif</option>
                            <option value="nonaktif" @selected(old('status', $kurir->status) === 'nonaktif')>Nonaktif</option>
                        </select>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="catatan" rows="3" class="mt-1 block w-full rounded-lg border-[#F5A623] text-sm text-[#2C1810] shadow-sm focus:border-[#E8612A] focus:ring-[#E8612A]">{{ old('catatan', $kurir->catatan) }}</textarea>
                        @error('catatan')<p class="mt-1 text-xs text-rose-600">{{ $message }}</p>@enderror
                    </div>

                    <div class="md:col-span-2">
                        <button type="submit" class="btn-primary">Update Kurir</button>
                    </div>
                </form>
            </div>
        </div>
