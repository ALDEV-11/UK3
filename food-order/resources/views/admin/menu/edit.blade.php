...
@section('content')
    <div class="py-6" style="background-color: #FFF8F3;">
        <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 space-y-4">
            <a href="{{ route('admin.menu.show', $menu->id_menu) }}" class="text-sm font-semibold text-[#2C1810] hover:text-[#E8612A]">← Kembali ke detail menu</a>
            ...
            <div class="overflow-hidden rounded-2xl bg-white p-6 shadow-lg border border-[#F5A623]">
                ...
                <form method="POST" action="{{ route('admin.menu.update', $menu->id_menu) }}" enctype="multipart/form-data" class="grid grid-cols-1 gap-4">
                    @csrf
                    @method('PUT')
                        ...
                        <div>
                            ...
                            <select name="id_restoran" class="mt-1 block w-full rounded-lg border-[#F5A623] text-sm text-[#2C1810] shadow-sm focus:border-[#E8612A] focus:ring-[#E8612A]" required>
                                @foreach($restoranList as $restoran)
                                    <option value="{{ $restoran->id_restoran }}" @selected((int) old('id_restoran', $menu->id_restoran) === (int) $restoran->id_restoran)>{{ $restoran->nama_restoran }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="id_kategori" class="mt-1 block w-full rounded-lg border-[#F5A623] text-sm text-[#2C1810] shadow-sm focus:border-[#E8612A] focus:ring-[#E8612A]" required>
                                @foreach($kategoriList as $kategori)
                                    <option value="{{ $kategori->id_kategori }}" @selected((int) old('id_kategori', $menu->id_kategori) === (int) $kategori->id_kategori)>
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                        <div>
                            ...
                            <input type="number" min="0" step="0.01" name="harga" value="{{ old('harga', (float) $menu->harga) }}" class="mt-1 block w-full rounded-lg border-[#F5A623] text-sm text-[#2C1810] shadow-sm focus:border-[#E8612A] focus:ring-[#E8612A]" required>
