    ...
    <div class="py-6">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="flex items-center justify-between gap-3">
                <h2 class="text-lg font-semibold text-gray-900">Menu Restoran Saya</h2>
                <a href="{{ route('restoran.menu.create') }}" class="inline-flex items-center rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-500">+ Tambah Menu</a>
            </div>
                                ...
                                <tr>
                                    ...
                                    <td class="px-4 py-3 text-right">
                                        <div class="inline-flex items-center gap-3">
                                            <a href="{{ route('restoran.menu.show', $item->id_menu) }}" class="text-indigo-600 hover:text-indigo-700">Detail</a>
                                            <a href="{{ route('restoran.menu.edit', $item->id_menu) }}" class="text-amber-600 hover:text-amber-700">Edit</a>
                                            <form method="POST" action="{{ route('restoran.menu.destroy', $item->id_menu) }}" onsubmit="return confirm('Yakin hapus menu ini?')">
                                                @csrf
                                                @method('DELETE')
